...
class Model(object):
    ...
    def build(self):
        ...
        # for right network
            self.activations.append(self.layers[1]())
            for l in xrange(2, layers_num):
...
class GCN(Model):
    ...
    def _loss(self):
        ...
        with tf.name_scope("reg_feature_extractor"):
            ...
            for layer in self.layers[2:feature_layer_num]:
                for var in layer.vars.values():
        ...
        with tf.name_scope("reg_domain_predictor"):
            for layer in self.domain_layers:
                for var in layer.vars.values():
                    self.domain_regular_loss += 10 * FLAGS.weight_decay * tf.nn.l2_loss(var)
