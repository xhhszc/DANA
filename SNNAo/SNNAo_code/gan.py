...
class alignGAN():
	...
	def build_model(self,sess):
		...
		# for test
		self.l2r_acc,self.r2l_acc ,self.a,self.b= self.get_acc()
	...
	def get_acc(self):
		...
		a=(self.test_all_score_l2r)
		b=(self.test_all_score_r2l)

		for i in range(50):
			l2r_acc.append(tf.reduce_mean(tf.cast(tf.nn.in_top_k(self.test_all_score_l2r, self.node_right,i+1), tf.float32)))
			r2l_acc.append(tf.reduce_mean(tf.cast(tf.nn.in_top_k(self.test_all_score_r2l, self.node_left, i+1), tf.float32)))

		return l2r_acc,r2l_acc,a,b
