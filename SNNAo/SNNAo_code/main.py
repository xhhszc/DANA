...
def test_link(sess,batch_size,model,filename,filelen,begin_index,size,
	...
	mapleft=0
	mapright=0
	result_l2r = np.array([0.] * 50)
	result_r2l = np.array([0.] * 50)
	c=[1]*args.test_anchors_num
	d=[1]*args.test_anchors_num
	while True:
		if read_index >= size:
			break
		...
		result_l2r_temp,result_r2l_temp,a,b= sess.run([model.l2r_acc,model.r2l_acc,model.a,model.b],feed_dict = feed_dict)

		a1=np.array(a)
		b1=np.array(b)

		for i in range(batch):
			temp=np.argsort(-(a1[i]))
			c[i] = np.reshape(np.where(temp==test_anchor_right[i])[0],[-1])[0] + 1
                    #c[i]=temp[test_anchor_right[i]]+1
			mapleft=mapleft+1.0/c[i]

		for i in range(batch):
			temp=np.argsort(-(b1[i]))
                    #d[i]=temp[test_anchor_left[i]]+1
			d[i] = np.reshape(np.where(temp==test_anchor_left[i])[0],[-1])[0] + 1
			mapright=mapright+1.0/d[i]
                #for i in range(batch):
                #    temp=a1[i][test_anchor_right[i]]
                #    for j in range(args.right_nodes_num):
                #        if a1[i][j]>temp:
                #            c[i]=c[i]+1
                #    mapleft=mapleft+1.0/c[i]

                #for i in range(batch):
                #    temp=b1[i][test_anchor_left[i]]
                #    for j in range(args.left_nodes_num):
                #        if b1[i][j]>temp:
                #            d[i]=d[i]+1
                #    mapright=mapright+1.0/d[i]
	...
	mapleft=mapleft/args.test_anchors_num
	mapright=mapright/args.test_anchors_num



	result = (result_l2r+result_r2l)/(2*size)
	return result,summary,mapleft,mapright
...
def main(args):
	...
	for epoch_all in range(args.epoch_all):
		...
		#test
		if epoch_all % 10 == 0:
			result_test,summary,mapleft,mapright= test_link(sess,args.test_batch_size,model,
			...
			if result_test[49] > best_p_test:
				print("best p_test:" + str(epoch_all) +  '\t', result_test)
				print(mapleft)
                print(mapright)
				best_p_test = result_test[49]
