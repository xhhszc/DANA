...
def load_source_data(dataset_str):
    ...
    for network in network_list:
        ...
        pkl.dump(graph_dict, open("../../data/ind.{}.graph.{}".format(dataset_str, network), 'wb'))
    ...
    for data in data_list:
        ...
        pkl.dump(anchors, open("../../data/ind.{}.anchor.{}".format(dataset_str, data), 'wb'))
...
def preprocess_adj(adj, transpose=False):
    ...
    adj_normalized = normalize_adj_v2(adj + sp.eye(adj.shape[0]), transpose)
...
def construct_feed_dict(support_left, support_right, anchor_left_idx, anchor_right_idx,
    ...
    feed_dict.update({placeholders['support_left_out'][i]: support_left[0][i] for i in range(len(support_left[0]))})
    feed_dict.update({placeholders['support_left_in'][i]: support_left[1][i] for i in range(len(support_left[1]))})
    feed_dict.update({placeholders['support_right_out'][i]: support_right[0][i] for i in range(len(support_right[0]))})
    feed_dict.update({placeholders['support_right_in'][i]: support_right[1][i] for i in range(len(support_right[1]))})
...
def chebyshev_polynomials(adj, k, transpose=False):
    """Calculate Chebyshev polynomials up to order k. Return a list of sparse matrices (tuple representation)."""
    print("Calculating Chebyshev polynomials up to order {}...".format(k))
    if transpose:
        adj_normalized = normalize_adj_v2_transpose(adj)
    else:
        adj_normalized = normalize_adj_v2(adj)
